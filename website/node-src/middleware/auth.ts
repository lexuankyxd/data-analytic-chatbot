...
export function generateToken(user_identifier: string, id: string): string[] {
  const access_token = jwt.sign({ email: user_identifier, id }, SECRET_ACCESS_KEY, { expiresIn: "7d" });
  // const access_token = jwt.sign({ user_identifier }, SECRET_ACCESS_KEY, { expiresIn: '15m' });
  const refresh_token = jwt.sign({ email: user_identifier, id }, SECRET_REFRESH_KEY, { expiresIn: "7d" });
...
export function refreshAccessToken(token: string): string {
  ...
  const newToken = jwt.sign({ email: tkn.email, id: tkn.id }, SECRET_ACCESS_KEY, { expiresIn: '7d' });
  return newToken;
}
