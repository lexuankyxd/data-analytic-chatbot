...
import { MessageItem, CMessage, FileMessage } from "../types";
...
export default function ChatPage() {
  ...
  const handleFileUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    file.forEach((f) => {
      setChatMessages(prevMessages => [...prevMessages, { data: { name: f.name, size: "" + f.size } }])
    })
    ...
    setTimeout(() => {
      setChatMessages(prevMessages => [
        ...prevMessages,
        { data: { text: `I've received your file: ${"AA"}. How would you like me to help with this?`, sender: 'bot' } }
      ]);
    }, 1000);
  };
  ...
  const handleSendMessage = async () => {
    ...
    setChatMessages([...chatMessages, { data: { text: message, sender: 'user' } }]);
            ...
            <div className="flex-grow overflow-y-auto px-6 pb-6 message-container">
              <div className="space-y-4">
                {chatMessages.map((msg, index) => (
                  (typeof msg.data == typeof sampleCMessage) ?
                    <ChatMessage
                      key={index}
                      text={msg.data.text}
